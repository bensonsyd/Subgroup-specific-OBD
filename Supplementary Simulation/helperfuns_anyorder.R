...
data_gen <- function(FullDat, starting_data, alpha_0, alpha_1, alpha_2, alpha_3, beta_0, beta_1, beta_2, beta_3, beta_4, beta_5, dose0, dose1, correlation, p.tox.true, p.eff.true, doses){
  ...
  if (nrow(FullDat) < nrow(starting_data)) {
    ...
    if(!is.na(dose0) & !is.na(dose1)){
      FullDat$Subgroup <- factor(FullDat$Subgroup, levels=0:1)
      FullDat_add<- data.frame(Subgroup = sample(c(rep(0, 30-unname(table(FullDat$Subgroup)[1])), rep(1, 30-unname(table(FullDat$Subgroup)[2]))), 2), 
                               Dose = rep(NA,2),
                               DLT = rep(NA,2),
                               eff = rep(NA,2))
      for(i in 1:2){
        if(FullDat_add$Subgroup[i] == 0){
          FullDat_add$Dose[i] <- dose0
        }else if(FullDat_add$Subgroup[i] == 1){
          FullDat_add$Dose[i] <- dose1
        }
      }
    }else if(is.na(dose0)){
      if(unname(table(FullDat$Subgroup)[2]) == 29){
        FullDat_add<- data.frame(Subgroup = c(1, NA), 
                                 Dose = c(dose1, NA),
                                 DLT = rep(NA,2),
                                 eff = rep(NA,2))
      }else{
        FullDat_add<- data.frame(Subgroup = rep(1,2), 
                                 Dose = rep(dose1,2),
                                 DLT = rep(NA,2),
                                 eff = rep(NA,2))
      }
      ...
      if(unname(table(FullDat$Subgroup)[1]) == 29){
        FullDat_add<- data.frame(Subgroup = c(0, NA), 
                                 Dose = c(dose0, NA),
                                 DLT = rep(NA,2),
                                 eff = rep(NA,2))
      }else{
        FullDat_add<- data.frame(Subgroup = rep(0,2), 
                                 Dose = rep(dose0,2),
                                 DLT = rep(NA,2),
                                 eff = rep(NA,2))
      }
    ...
    if(is.null(p.tox.true) | is.null(p.eff.true)){
      if(is.null(correlation)){
        for(i in 1:2){
          if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 0){
            FullDat_add$DLT[i] <- rbinom(1,1,prob_logis(alpha_0+alpha_1*FullDat_add$Dose[i]))
            FullDat_add$eff[i] <- rbinom(1,1,prob_logis(beta_0+beta_1*FullDat_add$Dose[i]+beta_2*FullDat_add$Dose[i]^2))
          }else if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 1){
            FullDat_add$DLT[i] <- rbinom(1,1,prob_logis(alpha_0+alpha_1*FullDat_add$Dose[i]+alpha_2+alpha_3*FullDat_add$Dose[i]))
            FullDat_add$eff[i] <- rbinom(1,1,prob_logis(beta_0+beta_1*FullDat_add$Dose[i]+beta_2*FullDat_add$Dose[i]^2 + beta_3 + beta_4*FullDat_add$Dose[i] +beta_5*FullDat_add$Dose[i]^2))
          }
        }
      }else{
        for(i in 1:2){
          if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 0){
            margprob0 <- c(prob_logis(alpha_0+alpha_1*FullDat_add$Dose[i]), prob_logis(beta_0+beta_1*FullDat_add$Dose[i]+beta_2*FullDat_add$Dose[i]^2))
            ...
            FullDat_add$DLT[i] <- ifelse(outcome == 1 | outcome == 2, 0, 1)
            FullDat_add$eff[i] <- ifelse(outcome == 1 | outcome == 3, 0, 1)
          }else if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 1){
            margprob1 <- c(prob_logis(alpha_0+alpha_1*FullDat_add$Dose[i]+alpha_2+alpha_3*FullDat_add$Dose[i]), prob_logis(beta_0+beta_1*FullDat_add$Dose[i]+beta_2*FullDat_add$Dose[i]^2 + beta_3 + beta_4*FullDat_add$Dose[i] +beta_5*FullDat_add$Dose[i]^2))
      ...
      for(i in 1:2){
        if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 0){
          FullDat_add$DLT[i] <- rbinom(1,1,p.tox.true[1:length(doses)][which(doses == FullDat_add$Dose[i])])
          FullDat_add$eff[i] <- rbinom(1,1,p.eff.true[1:length(doses)][which(doses == FullDat_add$Dose[i])])
        }else if(!is.na(FullDat_add$Subgroup[i]) & FullDat_add$Subgroup[i] == 1){
          FullDat_add$DLT[i] <- rbinom(1,1,p.tox.true[(length(doses)+1):(2*length(doses))][which(doses == FullDat_add$Dose[i])])
          FullDat_add$eff[i] <- rbinom(1,1,p.eff.true[(length(doses)+1):(2*length(doses))][which(doses == FullDat_add$Dose[i])])
        }
      }
    }
